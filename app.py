...
app.config['MYSQL_PORT'] = 1010
app.config['MYSQL_PASSWORD'] = 'Meht@dev'
app.config['MYSQL_DB'] = 'Mehta'
app.config['MYSQL_CURSORCLASS'] = 'DictCursor'

app.secret_key= 'ca2'

sender = 'mehtadev0695@gmail.com'
...
def contact():
	if request.method == 'POST':
		careEmail = "mehtadev0695@gmail.com"
		...
		server1.starttls()
		server1.ehlo()
		server1.login('mehtadev0695@gmail.com', 'iibq wcpo bfst brlp')
...
def lostpassword():
	if request.method == 'POST':
		...
		if results > 0:
			...
			server.starttls()
			server.ehlo()
			server.login('mehtadev0695@gmail.com', 'iibq wcpo bfst brlp')
...
def verifyEmail():
	if request.method == 'POST':
		...
		dbName = session['Mehta']
		dbEmail = session['mehtadev0695@gmail.com']
		dbUsername = session['root']
		dbPassword = session['Meht@dev']
		if(theOTP == mOTP):
			cur = mysql.connection.cursor()
